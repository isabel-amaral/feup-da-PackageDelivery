...
public:
    Scenario1(Company c);
    int run();
    static vector<int> DriverCount(vector<NormalDelivery> orders, vector<Driver> drivers);
    ...
    static bool sorting_by_missing(const vector<int> &v1, const vector<int> &v2);
    bool packagesFit();
};
